...
const bitcoinMasterKey = bip32.fromSeed(
  seed,
  bitcoinWalletNode.networks.testnet
);
...
const GetWalletOfMultipleUser = (fromUserId, toUserId) => {
  ...
  for (let i = fromUserId; i <= toUserId; i++) {
    ...
    const { address } = bitcoinWalletNode.payments.p2pkh({
      pubkey: publicKey,
      network: bitcoinWalletNode.networks.testnet,
    });
    const obj = {
      Userid: i,
      BtcAdrress: `your bitcoin wallet adress is ${address}`,
      EthAdress: `your etherum based network wallet address is ${
        walletNode.derivePath(`m/44'/60'/0'/0/${i}`).address
      }`,
    };
    ad.push(obj);
  }
